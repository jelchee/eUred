...
## Data Ingestion & Demo Data Entry

### Purpose

The pilot must demonstrate not only how a completed Battery Passport looks, but also how battery, compliance, lifecycle and supplier data enter the platform.

Because the pilot does not use real Rimac systems or real supplier data, all data entry must be simulated with synthetic but realistic data. The goal is to make the demo feel operationally credible and to show how the platform could later integrate with Rimac PLM, MES, ERP, BMS, QMS, supplier systems and OEM systems.

The platform must clearly show that all demo records are synthetic and that the system is running in **Demo Mode**.

---

## Data Entry Principles

1. The platform must not rely only on static pre-filled dashboards.
2. Users must be able to create, edit, import, review and approve demo data.
3. Data entry must simulate real enterprise workflows.
4. Every change must create an audit trail event.
5. Every data field must have a visibility level:

   * Public
   * Restricted
   * Confidential
6. Every important compliance item must have a status:
   ...
   * Rejected
   * Expired
7. The platform must distinguish between:

   * Manually entered data
   * CSV-imported data
   * Mock API-imported data
   * Telemetry-simulated data
   * Supplier-submitted data
   * Document-extracted or document-linked data

---

## Required Demo Data Entry Modules

### 1. Create Battery Asset

The platform must include a screen or modal for creating a new battery asset.

The form must support the following fields:

| Field                 | Example                     | Required | Visibility   |
| --------------------- | --------------------------- | -------: | ------------ |
| Asset ID              | SINE-HR-ZG-001              |      Yes | Restricted   |
| Serial Number         | RE-SN-2026-000145           |      Yes | Confidential |
| Product Family        | Rimac Energy SineStack      |      Yes | Public       |
| Battery Type          | Industrial BESS             |      Yes | Public       |
| Capacity              | 868 kWh                     |      Yes | Public       |
| Chemistry Category    | LFP                         |      Yes | Public       |
| Manufacturing Date    | 2026-04-18                  |      Yes | Restricted   |
| Manufacturing Site    | Rimac Campus, Croatia       |      Yes | Restricted   |
| Customer / Project    | Demo Industrial Site Zagreb |       No | Confidential |
| Installation Location | Zagreb, Croatia             |       No | Restricted   |
| Lifecycle Status      | Commissioned                |      Yes | Public       |
| Passport Status       | Draft                       |      Yes | Restricted   |

The user must be able to save the asset as draft.

After saving, the platform must generate:

* a private asset profile,
* a passport draft,
* an audit trail event,
* an initial compliance completeness score.

---

### 2. CSV / Excel Import for Battery Assets

The platform must include a demo import screen where the user can upload or paste CSV data.

The platform does not need to process real files in the prototype, but the UI must simulate the workflow.

Required workflow:

1. User selects “Import Battery Assets”.
2. User sees a downloadable CSV template.
3. User uploads or pastes demo CSV data.
4. Platform validates the data.
5. Platform shows preview rows.
6. User confirms import.
7. Platform creates assets and audit events.
...
* duplicate asset IDs,
* confidential fields,
* import source: “CSV Demo Import”.

---

### 3. Mock API Data Import

The platform must include a “Mock Integrations” screen that simulates importing data from Rimac enterprise systems.

Mock source systems:

| System                           | Simulated Data                                                            |
| -------------------------------- | ------------------------------------------------------------------------- |
| PLM                              | product model, battery chemistry, configuration, technical specifications |
| MES                              | serial number, manufacturing date, production batch, factory test result  |
| ERP                              | supplier references, purchase order references, component categories      |
| QMS                              | quality gate status, inspection report, test certificates                 |
| BMS Cloud / Battery Intelligence | SoC, SoH, cycle count, expected lifetime, degradation trend               |
| Document Vault                   | certificates, declarations, manuals, audit reports                        |
...
* update the passport draft,
* show which fields were populated,
* increase or decrease the passport completeness score,
...
The platform must include a telemetry simulator for demo purposes.

The simulator must generate realistic battery operating data for selected assets.
...
| State of Health             |             94–100% |
| Cycle Count                 |           120–2,400 |
| Average Module Temperature  |            22–39 °C |
| Round-trip Efficiency       |              89–93% |
| Availability                |         97.5–99.99% |
| Active Alarms               |                 0–4 |
| Last Communication          |           timestamp |
| Expected Remaining Lifetime |          7–14 years |
| Warranty Risk Score         | Low / Medium / High |

The user must be able to:

* start demo telemetry,
* pause demo telemetry,
* simulate normal operation,
* simulate warning condition,
* simulate critical alarm,
* reset telemetry to healthy state.

Example simulated telemetry event:

```json
{
  "assetId": "SINE-HR-ZG-001",
  "timestamp": "2026-06-24T10:15:00Z",
  "soc": 72.4,
  "soh": 98.7,
  "cycleCount": 842,
  "averageModuleTemperature": 29.6,
  "roundTripEfficiency": 91.8,
  "availability": 99.94,
  "activeAlarms": 0,
  "expectedRemainingLifetimeYears": 11.4,
  "warrantyRiskScore": "Low",
  "source": "Telemetry Simulator"
}
```

Telemetry data must be marked as synthetic.

---

### 5. Supplier Portal Demo

The platform must include a supplier portal or supplier data entry view.

Supplier users must only see their own obligations and submitted documents.

Demo supplier examples:

| Supplier                    | Component          | Required Evidence                                        |
| --------------------------- | ------------------ | -------------------------------------------------------- |
| Demo Cell Supplier A        | Battery cells      | Cell chemistry declaration, recycled content declaration |
| Demo Electronics Supplier B | BMS electronics    | RoHS/REACH declaration, safety certificate               |
| Demo Materials Supplier C   | Critical materials | Due diligence statement, origin declaration              |
| Demo Enclosure Supplier D   | Cabinet/enclosure  | Material composition declaration                         |

Supplier portal must support:

* viewing pending requests,
* uploading a document,
* entering structured data,
* submitting declaration for review,
* seeing approval status,
* seeing rejection reason.

Supplier-submitted data must not automatically become approved. It must go through compliance review.

---

### 6. Document Vault Upload

The platform must include a document upload / document vault screen.

For the prototype, actual file processing is not required, but the UI must simulate document upload and linking.
...
| Field               | Example                                  |
| ------------------- | ---------------------------------------- |
| Document ID         | DOC-2026-0145                            |
| Title               | Carbon Footprint Report — SineStack Demo |
| Type                | Carbon Footprint Report                  |
| Linked Asset        | SINE-HR-ZG-001                           |
| Linked Requirement  | Carbon Footprint Evidence                |
| Owner               | Rimac Compliance Team                    |
| Status              | Under Review                             |
| Expiry Date         | 2027-06-30                               |
| Visibility          | Restricted                               |
| Source              | Manual Upload                            |
| Synthetic Data Flag | Yes                                      |

---

### 7. Compliance Review Workflow

The platform must include a compliance review screen.

Compliance manager must be able to:

* view missing data,
* review submitted supplier documents,
* approve evidence,
* reject evidence,
* request changes,
* mark requirement as not applicable,
* publish passport when completeness threshold is reached.

Each compliance requirement must show:
...
| ---------------------------- | ------------ | -------------------------- |
| Battery model identification | Approved     | Rimac Engineering          |
| Chemistry category           | Approved     | Rimac Engineering          |
| Manufacturing date           | Approved     | Rimac Manufacturing        |
| Carbon footprint evidence    | Under Review | Rimac Compliance           |
| Recycled content declaration | Missing      | Supplier                   |
| Due diligence statement      | Submitted    | Supplier                   |
| Safety documentation         | Approved     | Rimac Compliance           |
| Dismantling instructions     | Draft        | Rimac Service              |
| Recycling instructions       | Draft        | Rimac Service              |
| BMS SoH data link            | Approved     | Rimac Battery Intelligence |

---

### 8. Passport Completeness Score

The platform must calculate and display a demo passport completeness score.
...
```json
{
  ...
  "manualFields": 8,
  "supplierFields": 5,
  "documentEvidenceItems": 12
}
```

The UI must explain why the passport is not yet publishable.

---

### 9. Public QR Passport Publishing

The platform must include a publishing workflow.

A public QR passport can only be published when:

* mandatory public fields are approved,
* required safety and recycling documents are approved,
* confidential fields are hidden,
* passport status is Approved.

Publishing workflow:

1. User opens passport draft.
2. Platform shows public preview.
3. Platform warns which fields will be public.
4. User confirms publishing.
5. Platform changes status to Published.
6. Platform generates public QR passport URL.
7. Audit trail records the publishing event.

The public QR passport must never show:

* supplier names unless explicitly marked public,
* detailed BOM,
* proprietary BMS parameters,
* internal warranty risk logic,
* customer confidential information,
* OEM confidential information.

---

### 10. Lifecycle Event Entry

The platform must include a form for adding lifecycle events.
...
```json
{
  "eventId": "LIFE-2026-0901",
  "assetId": "SINE-HR-ZG-001",
  "eventType": "Firmware Updated",
  "eventDate": "2026-06-18",
  "performedBy": "Rimac Service Team",
  "description": "Firmware updated to RE-BMS-4.8.2 during scheduled maintenance window.",
  "linkedDocumentId": "DOC-2026-0912",
  "visibility": "Restricted",
  "source": "Manual Entry"
}
```

Every lifecycle event must appear in the asset timeline.

---

## User Roles for Data Entry

The platform must support demo role switching.

| Role                     | Data Entry Permissions                                         |
| ------------------------ | -------------------------------------------------------------- |
| Public Viewer            | No data entry                                                  |
| Rimac Operator           | Create and edit battery assets                                 |
| Rimac Compliance Manager | Review, approve and publish passport data                      |
| Rimac Service User       | Add lifecycle and service events                               |
| Supplier User            | Submit supplier declarations and documents                     |
| ENT Platform Admin       | Configure integrations, data sources, roles and audit settings |
| ENT Security/Operations  | View system health, audit logs and data flow status            |
| Regulator                | View restricted compliance evidence, no editing                |
| Recycler                 | Add end-of-life and recycling events                           |

---

## Demo Flow Required for Presentation

The prototype must support the following scripted demo flow:

1. Start with an asset that has incomplete passport data.
2. Show passport completeness at 72%.
3. Import product and manufacturing data from mock PLM/MES.
4. Completeness increases to 82%.
5. Open supplier portal and submit missing recycled content declaration.
6. Compliance manager reviews and approves supplier evidence.
7. Completeness increases to 92%.
8. Start telemetry simulator and sync SoH/cycle data.
9. Completeness increases to 96%.
10. Preview private passport.
11. Preview public QR passport.
12. Publish public passport.
13. Show audit trail with every change.

This flow must be possible without any backend or real external systems.

---

## Functional Requirements

### FR-DI-001 — Create Asset Manually

The system shall allow authorized users to create a new battery asset using a structured form.

### FR-DI-002 — Save Asset as Draft

The system shall allow incomplete assets to be saved as drafts.

### FR-DI-003 — CSV Import Simulation

The system shall provide a CSV import workflow for creating multiple demo battery assets.

### FR-DI-004 — Import Validation

The system shall validate imported rows and show errors, warnings and duplicate asset IDs.

### FR-DI-005 — Mock API Import

The system shall simulate importing data from PLM, MES, ERP, QMS and BMS systems.

### FR-DI-006 — Telemetry Simulator

The system shall provide synthetic telemetry generation for selected battery assets.

### FR-DI-007 — Supplier Portal

The system shall allow supplier users to submit structured declarations and upload demo documents.

### FR-DI-008 — Document Vault

The system shall allow users to upload, classify, link and review demo documents.

### FR-DI-009 — Compliance Review

The system shall allow compliance managers to approve, reject or request changes for compliance evidence.

### FR-DI-010 — Passport Completeness Score

The system shall calculate and display a passport completeness score based on required data and evidence.

### FR-DI-011 — Public QR Preview

The system shall show a public passport preview before publishing.

### FR-DI-012 — Publish Passport

The system shall allow authorized users to publish an approved public QR passport.

### FR-DI-013 — Lifecycle Event Entry

The system shall allow authorized users to add lifecycle events to a battery asset.

### FR-DI-014 — Audit Trail

The system shall create an audit trail event for every create, edit, import, approval, rejection and publishing action.

### FR-DI-015 — Visibility Classification

The system shall classify every important field as Public, Restricted or Confidential.

---

## Acceptance Criteria

The demo data entry implementation is successful when:

1. A user can create a new demo battery asset manually.
2. A user can simulate importing multiple assets from CSV.
3. A user can simulate importing data from PLM, MES, ERP, QMS and BMS.
4. A user can start and stop telemetry simulation.
5. A supplier user can submit a missing declaration.
6. A compliance manager can approve submitted evidence.
7. Passport completeness changes based on user actions.
8. A private passport can be generated from entered data.
9. A public QR passport preview hides confidential data.
10. A passport can be published in demo mode.
11. The lifecycle timeline updates after new events.
12. The audit trail records all demo actions.
13. The UI clearly marks all data as synthetic demo data.
14. No real external systems, APIs or files are required.
